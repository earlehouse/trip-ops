...
export default async function RosterPage({ params }: { params: Promise<{ tripId: string }> }) {
  ...
  const [{ data: tripRaw }, { data: guestsRaw }, { data: teamsRaw }] = await Promise.all([
    ...
    supabase
      .from('guests')
      .select('id, trip_id, team_id, name, email, phone_number, arrival_date, arrival_time, departure_date, departure_time, hotel_confirmation, marriott_loyalty, hilton_loyalty, notes, team:teams(id, trip_id, name, headcount, color)')
      .eq('trip_id', tripId)
      .order('arrival_date')
      .order('arrival_time'),
