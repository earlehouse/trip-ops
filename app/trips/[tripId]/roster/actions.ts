...
export async function addGuest(tripId: string, teamId: string | null) {
  ...
  const { data, error } = await supabase
    ...
    .select('id, trip_id, team_id, name, email, phone_number, arrival_date, arrival_time, departure_date, departure_time, hotel_confirmation, marriott_loyalty, hilton_loyalty, notes')
    .single()
  if (error) throw new Error(error.message)
